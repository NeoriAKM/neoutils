...
- Весит 15 КБ (GNU ls последней версии — ~162 КБ).
- Работает **без** `stdio.h` & `string.h`. Только `unistd.h`
- Использует собственный `prints` & `strcmp`.
- Имеет дофига флагов, о которых красиво показано в **--help**

#### ***Если вам нужен баланс функциональности, веса и зависимостей — этот lsdr будет в самый раз.***
---
### dog - аналог cat

dog - аналог cat, но с сюрпризами.
...
Работает быстро, не спеша, символ не держа, спокойно, рубашкой шурша, весит копейки, много флагов:
- --help & --version : база
...
Внутри - свой strlen, strcmp, prints.
Я пытался добавить флаги, но.... не смог.. :P
...
Файлы лежат на сайте [NeoUtils](https://neoriakm.github.io/neoutils). Там есть статические, если у вас нету
`glibc`, и динамичные, если есть и вы хотите сэкономить пару сотен килобайт.